...
6. Create a deterministic preseason baseline mean using a recency-weighted blend of prior player seasons shrunk toward a position/stat cohort. Rookies or players without sufficient history do not train the primary residual model but remain a reported validation cohort when a baseline is available.
7. Do not use any current-season outcome or future-season field in a target season's features.
...
For every eligible player/stat target season `t`, construct a preseason baseline `mu_hat_t` using only information through `t-1`. The baseline is a configured recency-weighted blend of the player's prior one to three seasons, normalized for schedule length and shrunk toward the eligible position/stat cohort. Save every historical baseline in `historical_backtest_predictions.csv`.
